...
let error_message = document.querySelectorAll(".error__message");
...
function displayMessageSuccess() {
  ...
  let form_card = document.querySelector(".main__card__form");
...
function activeError(input, errorMessage, position) {
  input.classList.add('error');
  error_message[position].textContent = errorMessage;
  error_message[position].classList.add('active');
}

function clearError(input, position) {
  input.classList.remove('error');
  error_message[position].classList.remove('active');
}
...
function clearAll() {
  ...
  for (let i = 0; i < error_message.length; i++) {
    error_message[i].classList.remove('active');
  }
}
